...
cursor.execute('SELECT COUNT(*) FROM votes WHERE mac = %s AND entryno = %s', (macadr, entryno))
if cursor.fetchone()[0] == 0: #this mac address has not voted on this mii before
	...
	cursor.execute('SELECT ROUND(AVG(permlikes)) FROM mii WHERE craftsno=%s', [craftsno])

	average = cursor.fetchone()[0]

	if int(average) < 127:
		cursor.execute('UPDATE artisan SET popularity = %s, votes = votes+1 WHERE craftsno=%s', (average, craftsno)) #add +1 to total votes and set popularity to their average mii permlikes

	else:
		cursor.execute('UPDATE artisan SET votes = votes+1 WHERE craftsno=%s', [craftsno]) #add +1 to total votes
	...
	cursor.execute('INSERT INTO votes (mac, ip, entryno, craftsno) VALUES (%s, %s, %s, %s)', [macadr, ip, entryno, craftsno]) #log their vote
