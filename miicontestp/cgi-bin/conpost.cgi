...
miidata = form['miidata'].file.read().encode('utf-8', 'surrogateescape') #hack to base64 encode multipart form embedded binary
ip = escape(environ["REMOTE_ADDR"])
firstPost = True

cursor.execute('SELECT status FROM contests WHERE id = %s', [contestno])
if cursor.fetchone()[0] != 'open': #prevent posting on a contest not accepting new posts
	stdout.buffer.write(b"Content-Type:application/octet-stream\n\n")
	stdout.flush()
	stdout.buffer.write(bytes.fromhex('435000000000000000000000000000000000000000000000FFFFFFFFFFFFFFFF'))
	stdout.flush()
	exit()

if len(miidata) != 76: result(604)
if verifyMac(macadr) == False:	result(603)
if checkWiino(wiino) == False: result(611)
...
if not verified:	result(606) #mac address belongs to another artisan
...
cursor.execute('SELECT COUNT(*) FROM conmiis WHERE mac = %s AND contest = %s', (macadr, contestno))
if cursor.fetchone()[0] != 0:	firstPost = False #this MAC already posted to this contest

cursor.execute('SELECT COUNT(*) FROM conmiis WHERE craftsno = %s AND contest = %s', (craftsno, contestno))
if cursor.fetchone()[0] != 0:	firstPost = False #this artisan already posted to this contest

if firstPost: #this is a new contest entry
	cursor.execute('INSERT INTO conmiis (contest, craftsno, country, wiino, mac, ip, miidata) VALUES (%s, %s, %s, %s, %s, %s, %s)', (contestno, craftsno, country, wiino, macadr, ip, miidata))
